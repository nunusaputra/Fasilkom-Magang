...
const DospemLogbook = () => {
    ...
    const filterLogbook = Array.isArray(logbook) ? logbook.filter(logbook => search.toLowerCase() === "" || logbook.title.toLowerCase().includes(search.toLowerCase())) : []
    const lastPostIndex = currentPage * postPerPage
    const firstPostIndex = lastPostIndex - postPerPage
    const currentPost = filterLogbook.slice(firstPostIndex, lastPostIndex)
            ...
            <div className='bg-slate-50 rounded-lg drop-shadow-lg p-4'>
                <div className=''>
                    <div className='flex flex-col gap-2'>
                        <h1 className='text-lg font-bold'>Logbook Mahasiswa</h1>
                        <p className='text-sm text-slate-500'>You can see all logbook of your student here.</p>
                    </div>
                ...
                <div className='grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3'>
                    {isLoading ? (
                        ...
                        currentPost.length > 0 ? (
                            ...
                            <DataNotFound>
                                {search ? 'Your search not found' : 'Belum ada logbook'}
                            </DataNotFound>
                        )
                    )}
                </div>
