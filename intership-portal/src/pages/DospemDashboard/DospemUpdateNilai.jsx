...
import { updateNilai } from '../../redux/Action/NilaiAction'
import { toast } from 'react-toastify'
import { HashLoader } from 'react-spinners'

const DospemUpdateNilai = () => {
    const { id } = useParams()
    const dispatch = useDispatch()
    const navigate = useNavigate()
    const { bimbingan } = useSelector(state => state.bimbingan)
    const { nilai, isLoading } = useSelector(state => state.nilai)
            ...
            <div className='mt-10'>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className='flex flex-col-reverse gap-2 sm:justify-start sm:flex-row-reverse'>
                        ...
                        <Link to="/dospem-dashboard/nilai">
                            <button className='w-full px-4 py-2 border border-secondary text-secondary font-semibold rounded-lg hover:bg-secondary hover:text-white cursor-pointer'>
                                Cancel
                            </button>
                        </Link>
                    </div>
...
export default DospemUpdateNilai
