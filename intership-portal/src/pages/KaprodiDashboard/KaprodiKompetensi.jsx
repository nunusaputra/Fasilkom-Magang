...
const KaprodiKompetensi = () => {
    ...
    const filterKompetensi = Array.isArray(list) ? list.filter(kompetensi => search.toLowerCase() === '' || kompetensi.nama.toLowerCase().includes(search.toLowerCase())) : []
    ...
    useEffect(() => {
        ...
        const getList = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_URL_KAPRODI}/magang-kompetensi`, {
                    headers: {
                                ...
                                currentPost.length > 0 ? (
                                    ...
                                    <tr className=''>
                                        <td colSpan={50} className='text-center' style={{ height: '100px', verticalAlign: 'middle' }}>
                                            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                                                <DataNotFound>
                                                    {search ? 'Your search result not found' : 'Belum ada pengajuan magang komptenensi'}
                                                </DataNotFound>
                                            </div>
                                        </td>
                                    </tr>
...
export default KaprodiKompetensi
